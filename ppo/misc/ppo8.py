...
class ATCSchedulingEnv_b(gym.Env):
    """
    Custom Gym environment for Air Traffic Controller scheduling with random initial assignments.
    """
    ...
    def reset(self):
        ...
        # Reset schedule grid with random assignments
        self.schedule_grid = np.full((self.num_days, self.time_slots_per_day, self.num_positions), -1, dtype=int)
        self._initialize_random_schedule()
    ...
    def _initialize_random_schedule(self):
        # Initialize the schedule grid with random assignments
        for day in range(self.num_days):
            for time_slot in range(self.time_slots_per_day):
                controllers_assigned = set()
                for position in range(self.num_positions):
                    # Randomly decide whether to assign a controller or leave unassigned
                    if random.random() < 0.8:  # 80% chance to assign
                        # Get list of available controllers
                        available_controllers = self._get_available_controllers(day, time_slot, controllers_assigned)
                        if available_controllers:
                            controller_idx = random.choice(available_controllers)
                            self.schedule_grid[day, time_slot, position] = controller_idx
                            ...
                            # No available controllers, leave unassigned
                            pass
                    else:
                        # Leave the position unassigned
                        pass
                # Update controller states for break periods
                self._update_controller_breaks(controllers_assigned)
            # Reset daily hours at the end of the day
            self.controller_states[:, 4] = 0  # Reset Hours Worked Today

    def _get_available_controllers(self, day, time_slot, controllers_assigned):
        # Generate action mask for the current day and time slot
        mask = np.ones(self.num_controllers, dtype=bool)
        for c in range(self.num_controllers):
            # If the controller is on holiday today
            if day in self.controller_holidays[c]:
                mask[c] = False
                continue
            ...
            # Already assigned in this time slot
            if c in controllers_assigned:
                mask[c] = False
                continue

        available_controllers = np.where(mask)[0].tolist()
        return available_controllers

    def _update_controller_breaks(self, controllers_assigned):
    ...
    def step(self, action):
        ...
        if self.current_day >= self.num_days:
            ...
            total_reward = self.rewards['SUCCESSFUL_ASSIGNMENT'] * self.num_positions * self.time_slots_per_day * self.num_days - self.total_negative_reward
