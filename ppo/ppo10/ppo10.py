...
class ATCSchedulingEnv_b(gym.Env):
    ...
    def __init__(self):
        ...
        #                     Hours Worked This Week, Hours Worked Today, Needs Mandatory Break]
    ...
    def reset(self):
        ...
        self.total_unassigned_positions = 0
    ...
    def _get_observation(self):
        ...
        max_values = np.array([
            ...
            1.0                             # Needs Mandatory Break (0 or 1)
        ], dtype=np.float32)
    ...
    def step(self, action):
        ...
        for position_idx, controller_idx in enumerate(role_assignments):
            ...
            # Check if controller needs mandatory break
            if controller_state[5] == 1:
                # Controller was supposed to be on mandatory break
            ...
            # If controller has worked max continuous slots, set needs mandatory break
            if controller_state[1] >= self.max_continuous_work_slots:
                controller_state[5] = 1  # Needs Mandatory Break
        ...
        if len(not_assigned_controllers) > 0:
            ...
            # For controllers that need mandatory break
            needs_break_controllers = not_assigned_controllers[self.controller_states[not_assigned_controllers, 5] == 1]
            # For these controllers, check if mandatory break is completed
            for controller_idx in needs_break_controllers:
                if self.controller_states[controller_idx, 2] >= self.mandatory_break_slots:
                    # Mandatory break completed
                    self.controller_states[controller_idx, 5] = 0  # Reset needs mandatory break
                    # Reset Continuous Break Slots
                    self.controller_states[controller_idx, 2] = 0
        ...
        if self.current_day >= self.num_days:
            ...
            info['total_violations'] = self.total_violations + violation_count
            info['total_unassigned_positions'] = self.total_unassigned_positions + unassigned_positions_count
            info['total_non_violated_assignments'] = self.total_non_violated_assignments + non_violated_assignments_count
        else:
            self.done = False

        # Accumulate totals
        self.total_violations += violation_count
        self.total_unassigned_positions += unassigned_positions_count
        ...
        info['unassigned_positions'] = unassigned_positions_count
...
class EpisodeRewardLogger(BaseCallback):
    def __init__(self, log_dir, verbose=0):
        ...
        self.csv_writer.writerow(['Episode', 'Reward', 'Violations', 'Unassigned_Positions', 'Non_Violated_Assignments'])  # Header
    ...
    def _on_step(self) -> bool:
        if self.locals['dones'][0]:
            ...
            unassigned_positions = info.get('total_unassigned_positions', 0)
            non_violated_assignments = info.get('total_non_violated_assignments', 0)
            # Log to CSV
            self.csv_writer.writerow([self.episode_num, reward, violations, unassigned_positions, non_violated_assignments])
            if self.verbose > 0:
                print(f"Episode {self.episode_num} Reward: {reward}, Violations: {violations}, Unassigned Positions: {unassigned_positions}, Non-violated Assignments: {non_violated_assignments}")
            # Log to TensorBoard
            self.logger.record('episode/reward', reward)
            self.logger.record('episode/violations', violations)
            self.logger.record('episode/unassigned_positions', unassigned_positions)
            self.logger.record('episode/non_violated_assignments', non_violated_assignments)
