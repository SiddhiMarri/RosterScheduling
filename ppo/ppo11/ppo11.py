...
class ATCSchedulingEnv_b(gym.Env):
    ...
    def __init__(self):
        ...
        # Max continuous work sessions in slots (2 slots)
        self.max_continuous_work_slots = 2
        ...
        self.controller_states = np.zeros(
            (self.num_controllers, 6), dtype=np.float32)
        ...
        self.action_space = spaces.MultiDiscrete(
            [self.num_controllers + 1] * self.num_positions)
    ...
    def _get_observation(self):
        ...
        max_values = np.array([
            ...
            # Continuous Break Slots (max possible)
            self.max_continuous_work_slots,
    ...
    def step(self, action):
        ...
        if len(not_assigned_controllers) > 0:
            ...
            # Increase by 1 hour
            self.controller_states[not_assigned_controllers, 2] += 1
            # Reset Continuous Work Slots
            self.controller_states[not_assigned_controllers, 1] = 0

            # For controllers that need mandatory break
            needs_break_controllers = not_assigned_controllers[
                self.controller_states[not_assigned_controllers, 5] == 1]
            ...
            for controller_idx in needs_break_controllers:
                if self.controller_states[controller_idx, 2] >= self.mandatory_break_slots:
                    ...
                    # Reset needs mandatory break
                    self.controller_states[controller_idx, 5] = 0
        ...
        if self.current_day >= self.num_days:
            ...
            info['total_unassigned_positions'] = self.total_unassigned_positions
...
class EpisodeRewardLogger(BaseCallback):
    def __init__(self, log_dir, verbose=0):
        ...
        self.csv_writer.writerow(['Episode', 'Reward', 'Violations',
                                 'Unassigned_Positions', 'Non_Violated_Assignments'])  # Header
    ...
    def _on_step(self) -> bool:
        if self.locals['dones'][0]:
            ...
            self.csv_writer.writerow(
                [self.episode_num, reward, violations, unassigned_positions, non_violated_assignments])
            if self.verbose > 0:
            ...
            self.logger.record(
                'episode/unassigned_positions', unassigned_positions)
            self.logger.record(
