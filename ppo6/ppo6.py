...
class ATCSchedulingEnv_b(gym.Env):
    ...
    def __init__(self):
        ...
        self.num_controllers = 200  # Increased controllers to 50
    ...
    def step(self, action):
        ...
        if len(not_assigned_controllers) > 0:
            ...
            # Check for mandatory break after working 2 continuous slots
            for controller_idx in not_assigned_controllers:
                # If the controller had just worked 2 continuous slots
                if self.controller_states[controller_idx, 1] == 0 and self.controller_states[controller_idx, 2] == 1:
                    # Controller is taking a break after working
                    pass
                elif self.controller_states[controller_idx, 1] == 0 and self.controller_states[controller_idx, 2] < self.mandatory_break_slots:
                    # Controller hasn't taken the mandatory break yet
        ...
        if self.current_day >= self.num_days:
            ...
            total_reward = - self.total_negative_reward  # Best reward is 0
